...
def random_flip_along_x(gt_boxes, points, points_lidar, return_flip=False, enable=None):
    """
    Args:
    ...
    if enable:
        ...
        if points_lidar is not None:
            points_lidar[:, 1] = -points_lidar[:, 1]
        if gt_boxes.shape[1] > 7:
            gt_boxes[:, 8] = -gt_boxes[:, 8]
    if return_flip:
        return gt_boxes, points, enable
    return gt_boxes, points, points_lidar
...
def global_scaling(gt_boxes, points, points_lidar, scale_range, return_scale=False):
    """
    Args:
...
def random_translation_along_x(gt_boxes, points, points_lidar, offset_range):
    """
    Args:
...
def random_translation_along_y(gt_boxes, points, points_lidar, offset_range):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),

...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        if self.dataset_cfg.get('POINT_FEATURE_ENCODING_LIDAR', None) is not None:
            self.point_feature_encoder_lidar = PointFeatureEncoder(
                self.dataset_cfg.POINT_FEATURE_ENCODING_LIDAR,
                point_cloud_range=self.point_cloud_range
            )
        ...
        self.grid_size_lidar = self.data_processor.grid_size_lidar
        self.voxel_size_lidar = self.data_processor.voxel_size_lidar
    ...
    def prepare_data(self, data_dict):
        ...
        if hasattr(self, 'point_feature_encoder_lidar'):
            data_dict = self.point_feature_encoder_lidar.forward(data_dict)
        # 按照配置文件预处理部分进行预处理
    ...
    @staticmethod
    def collate_batch(batch_list, _unused=False): # 拼接一个batch的数据
        ...
        for key, val in data_dict.items():
            try:
                ...
                elif key in ['voxels_lidar', 'voxel_num_points_lidar', 'voxels_bev_lidar',
                             'voxel_num_points_bev_lidar']:
                    ret[key] = np.concatenate(val, axis=0)
                elif key in ['voxels_bev_lidar_4x', 'voxel_num_points_bev_lidar_4x']:
                    ret[key] = np.concatenate(val, axis=0)
                elif key in ['points', 'voxel_coords', 'points_lidar', 'voxel_coords_lidar', 'voxel_coords_bev',
                             'voxel_coords_bev_lidar', 'voxel_coords_bev_lidar_4x']:
                    coors = [] # 处理后的坐标
                    for i, coor in enumerate(val): # val是一个mini batch的点云数据
                        coor_pad = np.pad(coor, ((0, 0), (1, 0)), mode='constant', constant_values=i)
                        coors.append(coor_pad)
                    ret[key] = np.concatenate(coors, axis=0)
                elif key in ['mos_label']:
                    ret[key] = np.concatenate(val, axis=0)

...
class PointFeatureEncoder(object):
    ...
    def forward(self, data_dict):
        ...
        if data_dict.get('points_lidar', None) is not None:
            data_dict['points_lidar'], use_lead_xyz = getattr(self, self.point_encoding_config.encoding_type)(
                data_dict['points_lidar']
            )
            ...
            if self.point_encoding_config.get('filter_sweeps', False) and 'timestamp' in self.src_feature_list:
                ...
                dt = np.round(data_dict['points_lidar'][:, idx], 2)
                max_dt = sorted(np.unique(dt))[min(len(np.unique(dt))-1, max_sweeps-1)]
                data_dict['points_lidar'] = data_dict['points_lidar'][dt <= max_dt]
        else:

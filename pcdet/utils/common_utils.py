...
def remove_ground_points(points, distance_threshold):
    xyz = points[:, :3]
    ...
    mask[inliers] = False
    non_g_points = points[mask] # 保留非地面点所有特征
    non_ground_points = pcd.select_by_index(inliers, invert=True)
    # print(f'{len(inliers)} points removed')

    _, inliers2 = non_ground_points.segment_plane(
        distance_threshold=distance_threshold,
        ransac_n=3,
        num_iterations=1000,
    )
    mask2 = np.ones(len(non_g_points), dtype=bool)
    mask2[inliers2] = False
    left_points = non_g_points[mask2]
    # print(f'{len(inliers2)} points removed')
    return left_points
